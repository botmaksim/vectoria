...
function createExpressionsStore() {
    ...
    const { subscribe, set, update } = writable<Expression[]>([
...
function createSlidersStore() {
    ...
    const { subscribe, update, set } = writable<Record<string, Slider>>({
    ...
    return {
        ...
        set,
        /**
         * @brief Ensures sliders exist for the given variable names.
         * @param names Array of required variable names.
         */
        requireVars: (names: string[]) => update(current => {
            let changed = false;
            const next = { ...current };
...
export const camera = writable<CameraState>({ x: 0, y: 0, zoom: 50 });
